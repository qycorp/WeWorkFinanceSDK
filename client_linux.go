...
//  	!=0 - 失败
//
func NewClient(corpId string, corpSecret string, privateKeys map[string]string) (Client, error) {
	ptr := C.NewSdk()
...
*      !=0 - 失败
 */
func (s *SdkClient) DecryptData(encryptRandomKey string, encryptMsg string, publickeyVer string) (msg ChatMessage, err error) {
	encryptKey, err := RSADecryptBase64(s.privateKeys[publickeyVer], encryptRandomKey)
	if err != nil {
		return msg, err
	}
